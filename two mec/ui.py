...
class Example(QWidget):
    ...
    def show_camera(self):
        ...
        for pla, i in enumerate(self.persons):
            if i.getY() > self.down_limit or i.getY() < self.up_limit:
                ...
                cv2.putText(self.image, str(i.getId()), (i.getX(), i.getY()), self.font, 0.5, i.getRGB(), 1, cv2.LINE_AA)
    ...
    def drawText(self, event, qp):
        ...
        qp.setFont(QFont('Impact', 130))
        qp.drawText(QRect(1227, 569, 0.3*self.pixmap.width(), 0.2*self.pixmap.height()), Qt.AlignHCenter | Qt.AlignVCenter, self.text_2)
        qp.drawText(QRect(1227, 777, 0.3*self.pixmap.width(), 0.2*self.pixmap.height()), Qt.AlignHCenter | Qt.AlignVCenter, self.text_1)
